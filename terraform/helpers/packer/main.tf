...
#Hashing the image directory and variables to identify logical changes that require a new image
locals {
  input_hashes    = <<EOF
${sha1(jsonencode(var.extra_vars))}
${sha1(file("${var.directory}/image.json"))}
EOF
  hash_appended_ami_name = "${var.ami_name}-${sha1(local.input_hashes)}"
  final_ami_name  = "${local.hash_appended_ami_name}"
}
...
cd ${var.directory} && packer build \
  ...
  -var ami_name=${local.final_ami_name} \
